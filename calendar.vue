...
export default {
    ...
    methods: {
        ...
        select(k1, k2, e) {
            ...
            if (this.range) {
                ...
                if( filterDay.length>0 ){   //再次点击选中状态，则取消状态，填充新的数组
                    this.multiDays=this.multiDays.filter(v=> {
                      return this.year !== v[0] || this.month !== v[1]-1 || this.days[k1][k2].day !== v[2]
                    })
                }else{ //填充新的数据
                    this.multiDays.push([this.year,this.month+1,this.days[k1][k2].day,k1,k2]);
                }
                this.days[k1][k2].selected = !this.days[k1][k2].selected
                this.$emit('select',this.multiDays,this.days)
                    console.log(1)
            } else {
                // 取消上次选中
    ...
    //自定义取消选中状态
     closeSelectFunc(k1,k2,year,month,days){
        console.log(year,month,days,this.year,this.month)
         console.log('multiDays',this.multiDays)
          var  group = [];
           this.days.forEach((item,i)=>{
               item.forEach((d,j)=>{
                        ...
                        this.multiDays = this.multiDays.filter(v =>{
                        //    console.log(v)
                        return year !== v[0] || month !== v[1] || days !== v[2]
                       })     
            ...
            //     }
                // this.days[k1][k2].selected = !this.days[k1][k2].selected 
                console.log('循环之后:',this.multiDays)
                this.$emit('select',this.multiDays)
                    //  console.log('取消',this.multiDays)  
